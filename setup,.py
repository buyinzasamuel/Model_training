...
for name, model in binary_models.items():
    ...
    if accuracy > best_binary_score:
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            plt.title('Feature Importance for Binary Classification (E-waste Detection)')
...
print("Model 2: Multi-class Classifier of all the 9 waste types")
...
for name, model in multi_models.items():
    ...
    if accuracy_multi > best_multi_score:
        ...
        if hasattr(best_multi_model, 'feature_importances_'):
            ...
            plt.title('Feature Importance for Multi-class Classification')
            plt.bar(range(len(importances[:15])), importances[indices[:15]])
            plt.xticks(range(len(importances[:15])), [feature_columns[i] for i in indices[:15]], rotation=45, ha='right')
            plt.tight_layout()
            plt.show()  
...
print(perf_df.sort_values(by='F1-Score', ascending=False))
...
print("\nBinary Prediction:", predict_waste_type(example_features, 'binary'))
print("\nMulti-class Prediction:", predict_waste_type(example_features, 'multi'))
...
print("\nBinary Prediction:", predict_waste_type(example_features_2, 'binary'))
print("\nMulti-class Prediction:", predict_waste_type(example_features_2, 'multi'))

print("\nTraining completed! Two models are ready")   
print("1. Binary Classifier for E-waste Detection")
print("2. Multi-class Classifier for all 9 waste types")